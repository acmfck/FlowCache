...
class DiffusionForcingPipeline:
    ...
    def __init__(
        ...
        self.transformer = get_transformer(dit_path, load_device, weight_dtype, use_kvrange)
    ...
    def extend_video(
        ...
        for i in range(n_iter):
            # ------------
            self.transformer.clear_teacache()
            ...
            self.transformer.previous_e0_even = [None]*self.transformer.num_groups
            self.transformer.previous_e0_odd = [None]*self.transformer.num_groups
            self.transformer.previous_residual_even = [None]*self.transformer.num_groups
            self.transformer.previous_residual_odd = [None]*self.transformer.num_groups
            ...
            self.transformer.skip_even =  [[] for _ in range(self.transformer.num_groups)]
            self.transformer.skip_odd =  [[] for _ in range(self.transformer.num_groups)]
            # ------------
            ...
            for groupe_idx in range(len(self.transformer.skip_even)):
                if len(self.transformer.skip_even[groupe_idx]) ==0:
                    print(f"block { groupe_idx+1 } not skip any step")
                else:
                    for skip_cnt_idx in range(len(self.transformer.skip_even[groupe_idx])):
                        print(f"block { groupe_idx+1 } skip at step {self.transformer.skip_even[groupe_idx][skip_cnt_idx]}")
    ...
    def __call__(
        ...
        self.transformer.use_kvrange = args.use_kvrange 
        self.transformer.use_compress = args.use_compress
        latent_height = height // 8
        latent_width = width // 8
        latent_length = (num_frames - 1) // 4 + 1
        ...
        if overlap_history is None or base_num_frames is None or num_frames <= base_num_frames:
            ...
            # long video generation
            base_num_frames = (base_num_frames - 1) // 4 + 1 if base_num_frames is not None else latent_length # 25
            overlap_history_frames = (overlap_history - 1) // 4 + 1 # 5
            # ------------
            self.transformer.overlap_frames = overlap_history_frames
            self.transformer.group_size = causal_block_size
            self.transformer.num_groups = base_num_frames // causal_block_size
            ...
            if hasattr(self.transformer, 'kv_cluster'):
                self.transformer.kv_cluster.kvrange = args.kvrange
                self.transformer.kv_cluster.budget_block = args.budget_block
                self.transformer.kv_cluster.budget = int(args.budget_block * causal_block_size * latent_height * latent_width / 4.0)  
